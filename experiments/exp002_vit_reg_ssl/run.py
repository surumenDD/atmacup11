# experiments/exp005_vit_reg_ssl/run.py
import os
import sys
...
EXP_SHORT_NAME = Path(__file__).parent.name  # exp005_vit_reg_ssl
...
class ExpConfig:
    ...
    # SSL pretrain で学習した encoder の ckpt
...
class AtmaDataset(Dataset):
    def __init__(
        ...
        # 正方形パディング
        tfms.append(T.Lambda(pad_to_square))
...
def create_vit_reg(
    ...
    """
    ViT backbone (pretrained=False) に回帰ヘッドを付ける。
    ssl_checkpoint_path があれば encoder 部分だけロードする。
    """
    # 外部 pretrain は禁止なので、必ず pretrained=False
    ...
    if ssl_checkpoint_path is not None and ssl_checkpoint_path != "":
        ckpt_path = Path(ssl_checkpoint_path)
        if ckpt_path.exists():
            LOGGER.info("Load SSL checkpoint from %s", ckpt_path)
            ckpt = torch.load(ckpt_path, map_location="cpu")
            state_dict = ckpt.get("state_dict", ckpt)

            # ViTSSLModule での "encoder.xxx" だけを取り出してロード
            encoder_state = {}
            for k, v in state_dict.items():
                if k.startswith("encoder."):
                    new_k = k[len("encoder."):]  # "encoder." を外す
                    encoder_state[new_k] = v
            missing, unexpected = encoder.load_state_dict(
                encoder_state, strict=False
            )
            LOGGER.info("Loaded SSL encoder state. missing=%s, unexpected=%s",
                        missing, unexpected)
        else:
            LOGGER.warning("ssl_checkpoint_path not found: %s",
                           ssl_checkpoint_path)

    # 回帰ヘッド
    head = nn.Linear(feat_dim, 1)

    class ViTRegHead(nn.Module):
    ...
    return ViTRegHead(encoder, head)
...
def train_one_fold(
    ...
    module = RegressionModule(
        ...
        ssl_checkpoint_path=cfg.exp.ssl_checkpoint_path,
    )
...
def predict_test(
    ...
    for fold, ckpt_path in fold_to_ckpt.items():
        ...
        module = RegressionModule.load_from_checkpoint(
            ...
            learning_rate=cfg.exp.learning_rate,
            weight_decay=cfg.exp.weight_decay,
            ssl_checkpoint_path=None,  # ここでは自分の ckpt 内の重みだけ使う
        )
...
def main(cfg: Config) -> None:
    ...
    exp_dir_name = Path(sys.argv[0]).parent.name  # exp005_vit_reg_ssl
    ...
    LOGGER.info(
        "Loaded sample submission: %s (shape=%s)",
        sample_sub_path,
        submission_df.shape,
    )
