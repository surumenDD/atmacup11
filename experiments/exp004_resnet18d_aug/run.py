...
class AtmaDataset(Dataset):
    def __init__(
        ...
        if is_train:
            # 2-a. 学習時: RandomResizedCrop + Flip
            tfms.append(
                T.RandomResizedCrop(
                    size=img_size,
                    scale=(0.8, 1.0),
...
class EMA:
    def __init__(self, model: nn.Module, decay: float = 0.999):
        """
        指定された model (ここでは self.model) に対する EMA を管理するクラス。
        """
        self.decay = decay
        self.shadow: Dict[str, torch.Tensor] = {}
        self.backup: Dict[str, torch.Tensor] = {}

        # 初期化時は param と同じデバイスでクローン
    ...
    def update(self, model: nn.Module):
        """
        optimizer によるパラメータ更新「後」の model を渡して呼ぶ想定。
        """
        for name, param in model.named_parameters():
    ...
    def apply_shadow(self, model: nn.Module):
        """
        一時的に EMA パラメータを model へ適用する。
        元のパラメータは backup に退避する。
        """
        self.backup = {}
        for name, param in model.named_parameters():
            if name in self.shadow:
                ...
                # 元パラメータを退避してから EMA を適用
                self.backup[name] = param.detach().clone()
                param.data.copy_(shadow.data)
                self.shadow[name] = shadow  # デバイス更新後の shadow を保持
...
def create_resnet18d_reg(model_name: str = "resnet18d") -> nn.Module:
    """
    timm の resnet18d を使った回帰モデル。
    """
    model = timm.create_model(
        model_name,
        pretrained=False,
        num_classes=1,  # 1 次元出力
        in_chans=3,
    )
    return model
...
class RegressionModule(pl.LightningModule):
    """
    resnet18d による回帰タスク（RMSE） + EMA 対応。
    """
    ...
    def __init__(
        ...
        # exp_cfg は巨大なので save_hyperparameters から除外
        self.save_hyperparameters(ignore=["exp_cfg"])

        self.model = create_resnet18d_reg(model_name=exp_cfg.model_name)
        self.loss_fn = nn.MSELoss()

        # EMA 準備
        self.use_ema = exp_cfg.use_ema
        self.ema_warmup_steps = exp_cfg.ema_warmup_steps
        self.ema_step = 0  # EMA を何回更新したか
        if self.use_ema:
            # self ではなく self.model に対して EMA を作る
            self.ema = EMA(self.model, decay=exp_cfg.ema_decay)

    # ---- EMA 利用可否の判定 ----
    def _ema_is_active(self) -> bool:
        """
        現時点で EMA を使うかどうか。
        - use_ema=False なら常に使わない
        - ema_step == 0 の間は「EMA がまともに存在しない」とみなして使わない
        → train / val / predict すべてこの条件で統一
        """
        return getattr(self, "use_ema", False) and getattr(self, "ema_step", 0) > 0
    ...
    def on_save_checkpoint(self, checkpoint: dict) -> None:
        if getattr(self, "use_ema", False):
            ...
            checkpoint["ema_step"] = self.ema_step
    ...
    def on_load_checkpoint(self, checkpoint: dict) -> None:
        if getattr(self, "use_ema", False) and "ema_shadow" in checkpoint:
            ...
            # EMA が存在するので、ema_step を 1 以上にしておく
            self.ema_step = int(checkpoint.get("ema_step", 1))
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx: int) -> None:
        """
        optimizer のパラメータ更新後に EMA を更新する。
        一定ステップ数（ema_warmup_steps）を超えるまでは更新しない。
        """
        if not getattr(self, "use_ema", False):
            return

        # warmup までは EMA 更新しない
        if self.global_step < self.ema_warmup_steps:
            return

        # ここまで来たら self.model に対して EMA を更新
