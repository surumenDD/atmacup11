...
def pad_to_square(image: Image.Image, fill: int | tuple[int, int, int] = 0) -> Image.Image:
    ...
    例:
        元画像: 300 x 200 → パディング後: 300 x 300 → 後段で Resize(img_size, img_size)
    """
    w, h = image.size
    if w == h:
        return image
    ...
    max_side = max(w, h)
    # PIL.Image.new の color は int か (R,G,B)
...
def compute_channel_mean_std(
    ...
    この実験では「長辺に合わせた正方形パディング → Resize → ToTensor」で統計を取る。
    """
    tfm = T.Compose([
    ...
    for p in tqdm(paths, desc="Compute mean/std (square padded)"):
        try:
            ...
            img = pad_to_square(img)  # ★ ここで正方形パディング
            x = tfm(img)
        except Exception:
            continue
...
class AtmaDataset(Dataset):
    """
    object_id からパスを作り、
    「長辺に合わせて正方形パディング → Resize(img_size,img_size) → 変換」
    を行う Dataset。
    """
...
def train_one_fold(
    ...
    wandb_logger = WandbLogger(
        ...
        name=f"exp003_resnet18d_padnorm_fold{fold}",
...
def main(cfg: Config) -> None:
    ...
    oof_path = output_dir / \
        f"oof_resnet18d_exp003_padnorm_{len(used_folds)}folds.csv"
    ...
    sub_path = output_dir / \
        f"submission_resnet18d_exp003_padnorm_{len(fold_to_ckpt)}folds.csv"
