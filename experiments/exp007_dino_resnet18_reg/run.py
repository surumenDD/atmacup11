...
class ExpConfig:
    ...
    backbone_name: str = "resnet18"     # 今回は DINO ResNet18
    ...
    dino_checkpoint_path: str = "./output/dino_resnet18_ep800/checkpoint.pth"
    dino_checkpoint_key: str = "teacher"
    dino_patch_size: int = 16  # ViT 用。ResNet では使わない

    # embed_dim は backbone_name から決める（ViT 用）
...
def create_dino_vit_backbone(cfg: ExpConfig) -> tuple[nn.Module, int]:
    """
    ViT 系 DINO backbone（必要なら流用する）。
    """
    ...
    if cfg.backbone_name == "vit_small":
        ...
        raise ValueError(f"Unsupported DINO ViT backbone: {cfg.backbone_name}")
    ...
    print(f"[DINO-ViT] Loaded weights from {ckpt_path}, msg: {msg}")
...
class ResNetBackbone(nn.Module):
    """
    torchvision resnet18 を DINO 用に再利用するためのラッパ。
    forward で 512 次元の特徴を返す。
    """

    def __init__(self, model: nn.Module):
        super().__init__()
        self.model = model
        # DINO 学習時と同様に、fc を特徴抽出として使いたいので Identity にする
...
def create_dino_resnet_backbone(cfg: ExpConfig) -> Tuple[nn.Module, int]:
    """
    DINO で事前学習した ResNet18 を読み込む factory。
    DINO の checkpoint (teacher/backbone.*) を torchvision.resnet18 にロードして使う。
    """
    # torchvision の resnet18 を作成
    base_model = tvm.resnet18(weights=None)
    ...
    for k, v in state_dict.items():
        # DINO 側では "module.backbone.xxx" のような名前になっている想定
        k = k.replace("module.", "")
        k = k.replace("backbone.", "")
        cleaned[k] = v

    msg = base_model.load_state_dict(cleaned, strict=False)
    print(f"[DINO-ResNet18] Loaded weights from {ckpt_path}, msg: {msg}")

    backbone = ResNetBackbone(base_model)
    ...
    def __init__(
        ...
        if exp_cfg.model_family == "resnet":
            ...
            if "resnet" in exp_cfg.backbone_name:
                # ★ ここが今回の DINO ResNet18 用
                self.backbone, feat_dim = create_dino_resnet_backbone(exp_cfg)
            else:
                self.backbone, feat_dim = create_dino_vit_backbone(exp_cfg)
        else:
...
def main(cfg: Config) -> None:
    ...
    # sample_submission.csv の object_id は使わず、target だけ出す
